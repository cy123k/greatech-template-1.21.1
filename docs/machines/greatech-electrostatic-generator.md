...
- current coil rendering is still baked model rendering, not BER
...
The generator itself still uses hand-authored JSON resources. The coil model family is generated by the dedicated wireless coil datagen provider because it needs tier texture substitution plus a composite split between solid body and translucent top.
