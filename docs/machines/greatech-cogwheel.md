...
Generated encased small cogwheel blockstates use Create's `axis`, `top_shaft`, and `bottom_shaft` pattern.
...
For `GreatechEncasedCogwheelBlock`, the renderer uses the small cogwheel `*_shaftless` partial instead.
The baked blockstate model contributes the Create casing shell and shaft caps, while the BER contributes the rotating Greatech material cogwheel.
...
Current small cogwheel behavior:
...
- the encased block keeps the original Greatech material, small cogwheel block entity type, and kinetic break limit
- wrenching an axis face toggles `top_shaft` / `bottom_shaft`
- sneak-wrenching removes the casing and restores the matching Greatech small cogwheel

Current large cogwheel behavior is still the first-pass bridge:

- `create:andesite_casing` can encase Greatech large cogwheels as Create andesite encased large cogwheels
- `create:brass_casing` can encase Greatech large cogwheels as Create brass encased large cogwheels
- the resulting large encased block is still Create-owned, so material identity and Greatech break limits are not preserved for large cogwheels yet
