...
- `canUseSmallCogwheelHelper(...)`: final pair rule

The final pair rule is what prevents `create:item -> create:block` interactions from being intercepted.

## Event Dispatcher

Create's original cogwheel helper lives in Create's item class, so Greatech cannot rely only on Greatech item classes if `create:cogwheel` should work on Greatech targets.
...
The small cogwheel helper only treats cogwheel states as cogwheel targets.

Do not add shaft states to `GreatechSmallCogwheelPlacementHelper#getStatePredicate()` just to support cogwheel-on-shaft placement. Catnip filters helpers by item and target state before calling `getOffset(...)`; adding shaft targets there makes shafts look like cogwheel-helper targets and can produce misleading previews.
