...
import com.jjjcfy.greatech.content.gas.turbine.GasTurbineBlock;
...
public final class GreatechBlocks {
    ...
    private static final GasTurbineTier[] REGISTERED_GAS_TURBINE_TIERS = {
            GasTurbineTier.LV
    };
    ...
    public static final DeferredBlock<Block>[] GAS_TURBINES = registerGasTurbines();
    ...
    public static final DeferredBlock<Block> LV_GAS_TURBINE = gasTurbine(GasTurbineTier.LV);
    ...
    public static final DeferredItem<BlockItem>[] GAS_TURBINE_ITEMS =
            registerBlockItems(REGISTERED_GAS_TURBINE_TIERS, GAS_TURBINES, GreatechBlocks::gasTurbineName);
    ...
    public static final DeferredItem<BlockItem> LV_GAS_TURBINE_ITEM =
            gasTurbineItem(GasTurbineTier.LV);
    ...
    public static DeferredBlock<Block> gasTurbine(GasTurbineTier tier) {
        return GAS_TURBINES[tier.configIndex()];
    }
    ...
    public static DeferredItem<BlockItem> gasTurbineItem(GasTurbineTier tier) {
        return GAS_TURBINE_ITEMS[tier.configIndex()];
    }
    ...
    @SuppressWarnings("unchecked")
    private static DeferredBlock<Block>[] registerGasTurbines() {
        DeferredBlock<Block>[] blocks = new DeferredBlock[GasTurbineTier.values().length];
        for (GasTurbineTier tier : REGISTERED_GAS_TURBINE_TIERS) {
            blocks[tier.configIndex()] = registerGasTurbine(gasTurbineName(tier), tier);
        }
        return blocks;
    }
    ...
    private static int tierIndex(Object tier) {
        ...
        if (tier instanceof GasTurbineTier turbineTier) {
            return turbineTier.configIndex();
        }
    ...
    private static String gasTurbineName(GasTurbineTier tier) {
        return tier.id() + "_gas_turbine";
    }
    ...
    private static DeferredBlock<Block> registerGasTurbine(String name, GasTurbineTier tier) {
        return BLOCKS.register(
                name,
                () -> new GasTurbineBlock(BlockBehaviour.Properties.of()
                        ...
                        .isSuffocating((state, level, pos) -> false)
                        .isViewBlocking((state, level, pos) -> false)
                        .lightLevel(state -> state.getValue(GasTurbineBlock.ACTIVE) ? 1 : 0)
                        .requiresCorrectToolForDrops(), tier));
    }
